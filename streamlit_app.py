...
# =============================================================================
# STATIC LAYOUT — rendered once, never re-rendered
# =============================================================================
st.markdown("""<div class="main-title">Urban Flow &amp; <span style='color:#00ff88'>Life-Lines</span></div>
...
# =============================================================================
# SIDEBAR — user controls only, no sim logic
# =============================================================================
with st.sidebar:
...
# =============================================================================
# LIVE FRAGMENT — only this part re-runs on each tick, page stays still
# =============================================================================

# =============================================================================
# MAP FRAGMENT — separate slow-refresh fragment so the map doesn't blink
# on every sim tick. Runs every 2s independently of the 1s metrics fragment.
# uirevision="map_stable" tells Plotly to do a smooth data update, not a
# full redraw, so zoom/pan state is preserved between updates.
# =============================================================================
@st.fragment(run_every="2s" if st.session_state.sim_running else None)
def map_fragment():
    ...
    signal_phases = batch_signal_optimizer(tick, st.session_state.T, st.session_state.vc,
...
@st.fragment(run_every="1s" if st.session_state.sim_running else None)
def live_dashboard():
    fleet = st.session_state.fleet
    tick  = st.session_state.tick
    ...
    if st.session_state.sim_running:
        ...
        if tick % 8 == 0:
            add_log(f"{'EVP ACTIVE' if evp_active else 'DR:'}{'' if evp_active else f'{dr_now:.1f}%'} Active:{stats['active_total']:,}", "evp" if evp_active else "ok")
        if tick % 20 == 0:
    ...
    st.markdown(f"""<div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:8px'>
      <div></div>
      <div style='text-align:right'>{badge}<br>
        <span style='font-family:Share Tech Mono,monospace;font-size:0.55rem;color:#3a5a6a'>
        TICK #{tick} . {stats["active_total"]:,}/{fleet["N"]:,} ACTIVE . Phi={phi_v:.1f}s</span>
    ...
    def mc(col, label, val, unit, sub, sub_cls, card_cls):
        with col:
            st.markdown(f"""<div class="metric-card {card_cls}">
              ...
              <div class="metric-sub {sub_cls}">{sub}</div></div>""", unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with inf_c:
            ...
            for i in np.where(fleet["is_evp"] & ~fleet["completed"])[0][:6]:
                ...
                evp_cards += f"""<div class="vcard emg">
                  ...
                  <div style="font-family:Share Tech Mono,monospace;font-size:0.56rem;color:#3a5a6a;margin-top:2px">
                    {jnc[:10]} → {jnx[:10]}<br>{et['label']} . {prog}%</div></div>"""
            st.markdown(f"""
            ...
            <div style="margin-top:10px">
              <div class="metric-card mc-blue" style="margin-bottom:5px"><div class="metric-label">Normal Active</div>
                <div class="metric-value" style="font-size:1.3rem">{stats['active_normal']:,}</div></div>
              <div class="metric-card mc-red"><div class="metric-label">EVP Active</div>
                <div class="metric-value" style="font-size:1.3rem">{stats['active_evp']:,}</div></div>
            </div>""", unsafe_allow_html=True)
    ...
    # --- TAB 3 ---
    with tab3:
        st.markdown(f'<div class="sec-header{"  red" if evp_junctions else ""}">Adaptive Signal Matrix . {"EVP ACTIVE" if evp_junctions else "Normal Operations"} . DR={dr_now:.1f}%</div>', unsafe_allow_html=True)
        cols_sig = st.columns(5)
        pc = {"GREEN":"#00ff88","RED":"#ff3344","AMBER":"#ffaa00"}
        pe = {"GREEN":"G","RED":"R","AMBER":"A"}
        for i,jn in enumerate(JNAMES):
            phase,timer,dr_jn,_ = signal_phases[jn]
            clr = pc[phase]; density = float(dens_arr[i])
            ...
            with cols_sig[i%5]:
                st.markdown(f"""<div class="sig-card" style="border-color:{clr}33">
                  ...
                  <div style="font-family:Share Tech Mono,monospace;font-size:0.5rem;color:#1a3a4a">rho={density:.0f}</div></div>""", unsafe_allow_html=True)
        st.markdown("---")
    ...
    with tab4:
        ...
        with t4c2:
            st.markdown(f"""<div class="equation-box">
              ...
              d/dt + d(v)/dx = 0<br>
              Greenshields: v=v_max(1-rho/rho_max)<br>
              q=rho.v_max.(1-rho/rho_max)<br>
              w_s=(q2-q1)/(rho2-rho1)<br><br>
